...
export default function CTASection() {
  return (
    <section id="cta" className="relative py-24 px-4" style={{ background: 'radial-gradient(ellipse at center, rgba(124,58,237,0.15) 0%, rgba(99,102,241,0.05) 40%, transparent 70%)' }}>
      {/* Floating sparkles */}
      ...
      <Sparkles className="floating-icon bottom-[25%] left-[15%] text-cyan-400/40 animate-float-slow hidden md:block" size={18} />

      <div className="relative z-10 max-w-3xl mx-auto text-center">
        <motion.div {...fadeUp}>
          <SectionBadge>Get Started Today</SectionBadge>
        </motion.div>
        <motion.h2 {...fadeUp} transition={{ ...fadeUp.transition, delay: 0.1 }} className="font-heading text-4xl sm:text-5xl md:text-6xl font-bold mt-6">
          <GradientText>Study smarter with AI</GradientText>
        </motion.h2>
        <motion.p {...fadeUp} transition={{ ...fadeUp.transition, delay: 0.2 }} className="text-text-muted text-lg mt-4">
        ...
        <motion.p {...fadeUp} transition={{ ...fadeUp.transition, delay: 0.4 }} className="text-xs text-text-muted mt-6 opacity-60">
